...
async function cmdInit(channel) {
  ...
  try {
    ...
    // Write flags to .claude-primer.toml (with_readme, git_mode)
    const tomlPath = path.join(cwd, ".claude-primer.toml");
    const tomlLines = [
      "# Generated by Claude Primer VS Code extension",
      "",
      "[flags]",
      `with_readme = ${withReadme === "Yes"}`,
      `git_mode = "${gitMode}"`,
    ];
    fs.writeFileSync(tomlPath, tomlLines.join("\n") + "\n", "utf8");

    channel.appendLine(`Wrote project config to ${rcPath}`);
    channel.appendLine(lines.join("\n"));
    channel.appendLine(`Wrote flags to ${tomlPath}`);
    channel.appendLine(tomlLines.join("\n"));
    channel.show(true);

    vscode.window.showInformationMessage(
      `Claude Primer: Config saved to .claude-setup.rc and .claude-primer.toml`
    );
  } catch (e) {
