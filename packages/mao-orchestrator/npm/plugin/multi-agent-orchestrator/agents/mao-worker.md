...
- Business logic with 3+ conditional branches or state transitions
- Any code touching auth, encryption, tokens, access control, or secrets
- Concurrency: locks, mutexes, channels, race conditions, async coordination
- Architectural decisions: new patterns, API design, schema changes with migration logic
- Code requiring understanding of >3 existing files to implement correctly
- Any task where you're unsure of the correct approach after reading the context
...
Skip TDD only for work with no runtime behavior: config files, type definitions,
documentation, formatting fixes, import updates, file moves/renames.
